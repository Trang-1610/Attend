...
export default function StudentManagement() {
    ...
    const getColumnSearchProps = (dataIndex) => ({
        ...
        onFilter: (value, record) => {
            ...
            if (typeof cellValue === 'boolean') {
                return (value.toLowerCase() === 'true' && cellValue === true) ||
                    (value.toLowerCase() === 'false' && cellValue === false);
            }
        ...
        },
        {
            ...
            render: (_, record) => (
                <div className="flex gap-2">
                    <Button size="small" onClick={() => openResetPasswordModal(record)}>Cấp lại mật khẩu</Button>
                    {record.is_locked ? (
    ...
    const openLockConfirmModal = (student) => {
        setStudentToLock(student);
        setLockConfirmVisible(true);
    };

    const openUnLockConfirmModal = (student) => {
        setStudentToUnLock(student);
        ...
        <Layout style={{ minHeight: '100vh' }}>
            ...
            <Layout>
                ...
                <main className="mx-4 my-4 p-4 sm:p-6 bg-white rounded shadow">
                    ...
                    <Modal
                        title="Cấp lại mật khẩu cho sinh viên"
                        ...
                        <div className="space-y-4">
                            <div>
                                <p className="font-medium text-gray-700">
                                    Gửi đến email: <strong className=''>{selectedStudent?.email}</strong>
                                </p>
                            </div>
                    ...
                    >
                        <p>Bạn có chắc chắn muốn tạm dừng tài khoản của sinh viên <strong>{studentToLock?.name}</strong>?</p>
                    </Modal>
                    ...
                    <Modal
                        ...
                        confirmLoading={isLocking}
                    >
                        <p>Bạn có chắc chắn muốn mở khoá tài khoản của sinh viên <strong>{studentToUnLock?.name}</strong>?</p>
                    </Modal>
