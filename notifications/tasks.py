...
@shared_task
def send_reminder_email(student_id, subject_id, email, when):
    """
    Send email (before 1h or 30m).
    """
    ...
    lesson_start = localtime(data["occurrence_start"], vn_tz)

    start_str = lesson_start.strftime("%H:%M %d/%m/%Y")

    # Render template
    html_content = render_to_string("reminder/remind_attendance_email.html", {
        "student_name": data["student_name"],
        "subject_name": data["subject_name"],
        "class_name": data["class_name"],
        "period_number": data["slot_name"],
        "start_time": start_str,
        "minutes_left": "60" if when == "1h" else "30",
    })

    send_mail(
        subject=f"Nhắc nhở điểm danh môn {data['subject_name']}",
        message="",
